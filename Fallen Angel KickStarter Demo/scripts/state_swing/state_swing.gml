...
repeat(abs(walk_speed * (right - left)))
{
    ...
    {
        if place_meeting(x - (other.right - other.left)*dcos(other.sword_angle), y, other)
            {
                if other.z >= height
                {
                    other.can_move = true;
    ...
    if can_move == true
        x += (right - left)*dcos(sword_angle);
	//spd*-dsin(sword_angle);
}
...
repeat(abs(walk_speed * (down - up)))
{
	...
	with (obj_cube_parent)
    {
        if place_meeting(x, y - (other.down - other.up)*-dsin(other.sword_angle), other)
            {
                if other.z >= height
                {
                    other.can_move = true;
...
if animation_hit_frame(2)
{
	...
	if z > z_ground {
		var _hitbox = create_hitbox(s_sword_hitbox_air, x, y-4, sword_angle, _life, [o_enemy, o_grass, o_bush, o_shrine], _damage, _knockback);
		with _hitbox {
			if !place_meeting(x,y,o_enemy)  {
				with other {
					/*var lenx = lengthdir_x(AIR_SWING_KNOCKFORWARD,o_reticle.imagesword_angle);
					var leny = lengthdir_y(AIR_SWING_KNOCKFORWARD,o_reticle.imagesword_angle);
		...
		switch power_stance {
			...
			case false:
			var _hitbox = create_hitbox(s_sword_hitbox, x, y-4, sword_angle, _life, [o_enemy, o_grass, o_bush, o_shrine], _damage, _knockback);
			break;
			
			case true:
			var _hitbox = create_hitbox(s_power_attack_hitbox, x, y-4, sword_angle, _life, [o_enemy, o_grass, o_bush, o_shrine], _damage, _knockback);
			break;
	...
	with _hitbox {
		if (place_meeting(x,y,o_solid) or place_meeting(x,y,o_lancer)) and !place_meeting(x,y,o_cherub) and !place_meeting(x,y,o_flameangel) and !place_meeting(x,y,o_missile_angel) and !place_meeting(x,y,o_tankangel) and !place_meeting(x,y,o_abdiel) and !place_meeting(x,y,o_solid_lesser) and other.z == other.z_ground     
		{
	...
	//set_movement(sword_angle, 2);
	
	/*spd*dcos(sword_angle);
	spd*-dsin(sword_angle);*/
...
if _evade_input >= .7 and anim_cancel and can_dash {
...
if state_ != player.sword3 {
	...
	if (_attack_input >= .7 and combo_ )	
	{
		...
		get_direction_facing(sword_angle);
		switch state_ {
